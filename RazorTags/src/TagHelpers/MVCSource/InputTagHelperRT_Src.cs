...
namespace RazorTags //Microsoft.AspNetCore.Mvc.TagHelpers
{
	...
	public partial class InputTagHelperRT : TagHelper
	{
		...
		/// <summary>
		/// Gets the <see cref="IHtmlGenerator"/> used to generate the <see cref="InputTagHelperRT"/>'s output.
		/// </summary>
		...
		{
			var arr = GetInputTypeHints(modelExplorer).ToArray();

			// foreach(var hint in GetInputTypeHints(modelExplorer)) {
			foreach(var hint in arr) {
			...
			IDictionary<string, object> htmlAttributes)
		{
			if(modelExplorer.ModelType == typeof(string)) {
				...
				ThrowExOnInvalidModelTypeShouldBeBool();
			}
			...
			if(string.IsNullOrEmpty(format)) {
				if(!modelExplorer.Metadata.HasNonDefaultEditFormat &&
					...
					modelExplorer = modelExplorer.GetExplorerForModel(
						GetFormattedWeek(modelExplorer)); //FormatWeekHelper.GetFormattedWeek(modelExplorer));
				}
				else {
		...
		private static IEnumerable<string> GetInputTypeHints(ModelExplorer modelExplorer)
		{
			...
			string[] names = GetTypeNames(modelExplorer.Metadata, fieldType).ToArray();
			// TemplateRenderer.GetTypeNames(modelExplorer.Metadata, fieldType).ToArray();
